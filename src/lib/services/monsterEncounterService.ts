...
export function getMonsterEncounterDays(cards: ClientSideCard[], monsters: Monster[], dayHours: ClientSideDayHours[]): MonsterEncounterDay[] {
    ...
    const monsterEncounterDays = monsterEncounterDayHours.map((dayHour) => {
        const groups = dayHour.spawnGroups.map(group => {
            const monsterEncounters = group.ids.map(cardId => {
